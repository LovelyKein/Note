...
Vue 在解析模版的时候会创建组件`my-component`的实例对象vc
即 Vue 会执行`new VueComponent(options) `创建一个组件实例对象vc
...
`$event`是 Vue 中的**事件对象**，相当于原生 JavaScript 函数中的 `event`
...
Vue 中的一个**插件**库，提供一个类似`axios`的方法去发送 Ajax 请求
